...
그다음은 동영상 시간 업데이트와 관련된 초기 initialize 함수입니다.
```typescript
  const addTimeUpdate = () => {
    const observedVideoElement = ref && ref.current;
    if (observedVideoElement) {
...
마지막으로는 하단 control bar의 visible과 관련된 함수입니다.
```typescript
  const handleControlVisible = () => {
    if (!showControl) {
      setShowControl(true);
...
기존 video tag에서 controls 옵션을 주어 보여지는 UI 영역에 해당합니다. 기존 video에서는 controls를 사용하면 컨트롤바가 보이지만 실제로 custom 하기 위해서는 컴포넌트를 제작해야 합니다.
...
controlbar 영역도 크게 두군데로 나뉠수 있는데 볼륨관련, progress 바 조정을 위한 마우스 up/down 함수부분입니다.
...
그다음은 크게 살펴볼 부분은 없고, 시간 출력 함수입니다.
```typescript
