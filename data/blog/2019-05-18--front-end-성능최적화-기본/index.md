...
성능측정을 하기위해서는 브라우저의 렌더링과정을 이해하는것이 필요합니다.
...
- 다운받은 html을 파싱해서 dom 생성 후 트리를 구축 하는것입니다.
- dom 트리를 구축하는 것 뿐만 아니라 `<script /> <link /> <img />` 등등 리소스 필요할때는 요청 및 다운로드를 합니다.
- cssom 트리 구축하는 과정도 포함됩니다.
...
- dom 과 cssom을 매칭해서 렌더트리에 그리는 과정입니다.

#### 3) 레이아웃 (reflow)

- 브라우저의 view port 안에서 노드의 위치와 크기 계산하여 렌더트리에 반영하는 과정입니다.
- 노드를 순회하면서 계산이 일어납니다.

#### 4) 페인트

- 레이아웃에서 계산된 값을 화면상의 실제 픽셀로 변환하는 과정입니다.
- 위치와 관계없는 background-color, color 등등의 css 속성들이 결정되는 과정입니다.

#### 5) 합성 & 렌더

- 페인트 된 레이어들을 합쳐 업데이트하는 과정입니다.
- css transform도 이과정에 속하게 됩니다.
...
- DOMContentLoaded
  - html ,css 파싱 끝난시점 (렌더트리 구축을 위한 준비 완료 시점)입니다.
- Loaded
  - html 상에 모든 리소스가 load 된 시점입니다.

브라우저 입장에서는 위의 DOMContentLoaded 와 Loaded 가 빠르면 페이지의 리소스들을 빠르게 준비할수 있다는 것을 말하므로 최적화가 되었다고 말할수 있습니다.

#### 2)**사용자 관점** 에서의 로딩 최적화

위의 리소스 로딩시간은 사용자의 기기성능이나 네트워크 조건에 따라 다를 수 있습니다.
...
- 최적화
  ...
  - 초기 렌더링에 쓰이지 않는 스크립트는 `defer` , `async` 속성을 명시해서 blocking을 방지하게 됩니다.
...
- `link rel preload` 사용 하기
  - 글꼴이라던지, 초기렌더링 전에 반드시 불러와야하는 중요한 css나 js 파일을 미리 불러옵니다
...
- js 코드로 동적으로 html 요소의 스타일이나 css 클래스를 변경시킬때
  - render tree는 dom tree와 cssom tree를 맵핑시켜 만들어지므로 위에서 코드로 이를 변경시킴으로 인해 다시 레이아웃을 구축해야 하는 상황이 발생합니다.

#### 2)reflow와 repaint

- reflow
  - 모든 엘리먼트의 위치나 길이, 크기 등등을 다시 계산하는 과정입니다.
  - 상위 엘리먼트를 변경시키면 하위 엘리먼트에도 영향을 끼칩니다.
  - render tree를 재생성하므로 부하가 큽니다.
  - 레이아웃에 영향을 줍니다.
- repaint
  - 레이아웃에 영향을 주지않지만 눈에 보이는 요소들(background-color, color, visibility,..)이 변경됩니다.
  - reflow 보다는 부하가 크지는 않습니다.

##### reflow가 일어나는 상황

- dom을 추가하거나 삭제할때 발생합니다.
- css 속성중 높이, 넓이, 위치 에 영향을 주는 속성값을 변경할때 발생합니다.
...
- css 속성중 높이, 넓이, 위치등에 영향을 주지않는 속성을 변경할때 발생합니다.
